...
`GENMOD`, `ROBUSTREG`, `MIXED`, `PLS`, and `HPSPLIT` (`NLIN` uses its own
`MODEL y = <expr>;` shape, described above, not this shared one).
...
- PROC steps beyond `PRINT`/`CONTENTS`/`SORT`/`MEANS`/`SUMMARY`/`FREQ`/`APPEND`/
  ...
  `TIMESERIES`/`PLS`/`NLIN`/`HPSPLIT` raise a clear
